...
class STLProcessorPublisher(Node):
    def __init__(self):
        super().__init__('stl_processor_publisher')
        self.declare_parameter('topic_name', 'sample_pointcloud')
        self.declare_parameter('filename', 'wind_0.stl')
        ...
        results_dir = "/workspaces/isaac_ros-dev/src/osep/stl_models"
        results_dir = "/home/airlab/workspaces/isaac_ros-dev/src/osep/stl_models"
        self.stl_path = os.path.join(results_dir, filename)
        ...
        self.get_logger().info(f"Publishing {self.stl_path} as point cloud on topic: {topic}")

        # Load the STL mesh and sample points
        mesh = o3d.io.read_triangle_mesh(self.stl_path)
        if not mesh.has_triangles():
            raise RuntimeError(f"Mesh {self.stl_path} has no triangles!")
        voxel_grid = o3d.geometry.VoxelGrid.create_from_triangle_mesh(mesh, voxel_size=self.voxel_size)
        self.points = np.array([voxel.grid_index * self.voxel_size + voxel_grid.origin + self.voxel_size / 2
                                for voxel in voxel_grid.get_voxels()], dtype=np.float32)
        self.colors = np.tile(np.array([[1.0, 0.0, 0.0]], dtype=np.float32), (self.points.shape[0], 1))  # All red

        self.get_logger().info(f"Voxelized mesh: {len(voxel_grid.get_voxels())} voxels at size {self.voxel_size}")
    ...
    def timer_callback(self):
        ...
        for i in indices:
            ...
            if raycast_occluded(pt, drone_pos, points_voxels, self.voxel_size):
                continue
            if not np.all(np.isclose(self.colors[i], [0.0, 1.0, 0.0])):
